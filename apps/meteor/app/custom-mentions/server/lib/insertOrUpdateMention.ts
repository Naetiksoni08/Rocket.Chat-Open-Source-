...
export async function insertOrUpdateMention(userId: string | null, mentionData: CustomMentionData): Promise<CustomMentionData> {
	...
	if (!mentionData.usernames || mentionData.usernames.length === 0) {
		throw new Meteor.Error('error-the-field-is-required', 'At least one member is required');
	}
	...
	if (!mentionData._id) {
		...
		const result = await MentionCustom.create({
			...
			usernames: mentionData.usernames,
		});
